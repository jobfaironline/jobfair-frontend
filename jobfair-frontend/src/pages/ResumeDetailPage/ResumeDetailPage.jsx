...
const ResumeDetailPage = () => {
  const { id } = useParams();
  const { roles: role, userId } = useSelector((state) => state.authentication.user);

  let container = <ResumeDetailForCompanyContainer resumeId={id} />;
  if (role === RoleType.ATTENDANT) container = <ResumeDetailForAttendantContainer resumeId={id} attendantId={userId} />;
