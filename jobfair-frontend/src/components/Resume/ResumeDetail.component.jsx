...
const ResumeDetailComponent = props => {
  ...
  const result  = data.candidateEducation?.map(item => item.qualificationId).map(name => {
  ...
  const adwards = !!data?.candidateCertifications ? data.candidateCertifications.map((item, index) => {
    return {
      id: index,
      time: item.year,
      titleName: item.name,
  ...
  const skills = !!data?.candidateSkills ? data.candidateSkills.map((item, index) => {
    return {
      id: index,
      name: item.name
    }
  ...
  const references = !!data?.candidateReferences ? data.candidateReferences.map((item, index) => {
    return {
      id: index,
