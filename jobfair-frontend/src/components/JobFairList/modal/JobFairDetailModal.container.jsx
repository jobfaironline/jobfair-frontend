...
import CompanyRegistrationDetailModalComponent
    from "../../ApprovalRegistration/modal/CompanyRegistrationDetailModal.component";
import JobFairDetailModalComponent from "./JobFairDetailModal.component";
import {getAccountByIdAPI} from "../../../services/userService";
...
const JobFairDetailModalContainer = ({jobFairId, creatorId, visible, setModalVisible, jobFairList}) => {
    ...
    const fetchData = async () => {
        ...
        getAccountByIdAPI(creatorId)
            .then(res => {
                setCreatorInfo(`Full name: ${res.data.firstname} ${res.data.middlename} ${res.data.lastname}. Role: ${res.data.role}`)
            })
            .catch(err => {
                notification['error'] ({
