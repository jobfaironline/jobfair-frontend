...
export const fixTextureOffset = (mesh) => {
  if (mesh?.material?.map !== null) mesh?.material?.map?.center.set(0.5, 0.5);
...
export const extractTexture = (mesh, meshName) => {
  ...
  if (mesh.material.map !== null && mesh.material.map.isVideoTexture) {
    result.push({ texture: mesh.material.map, meshName });
    const newMaterial = mesh.material.clone();
    newMaterial.map = null;
    mesh.material = newMaterial;
  }

  for (const child of mesh.children) result = [...result, ...extractTexture(child, meshName)];
